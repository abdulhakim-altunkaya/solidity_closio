...
function OwnClosioChange() {
  ...
  const changeOwner = async () => {
    try {
      ...
      //Execution
      let ownerChangeVar = await contractClosio.transferOwnership(newOwner);
      await ownerChangeVar.wait(); 
      setMessage("Owner Closio changed");
      ...
      if (error.transaction && error.transaction.from) {
        ...
        alert("Closio ownership change failed");
...
export default OwnClosioChange;